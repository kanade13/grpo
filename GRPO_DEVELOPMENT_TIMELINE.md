...
| Day 3 | 实现 `compute_group_advantages`。对每个 prompt group 的 reward 做均值、标准差和 advantage 计算。 | 新增 advantage 单元测试，覆盖普通 reward、全相等 reward、单元素 group、负 reward；确认全相等时 advantages 为 0。 | 已完成 |
| Day 4 | 实现 `build_response_mask`。构造只覆盖 response tokens 的 mask，排除 prompt tokens 和 padding tokens。 | 新增 mask 单元测试，覆盖 right padding、left padding、不同 prompt length、短 response、全 padding 边界。 | 已完成 |
